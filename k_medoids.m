...
% X: the data matrix, rows are data points and min-indexumns are features
...
while true
    ...
    % You may find the Matlab function pdistancet2 to be useful here. 
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for j = 1:n_cluster
        ...
        distance = [];
        for i = 1:size(temp)
            distance(i) =sum(pdist2(temp,temp(i,:),"cityblock"));
        end
        [~,min_index] = min(distance);
        centres(j,:) = temp(min_index,:);
    end
